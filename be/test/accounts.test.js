...
async function signupUser(app, client, u, a, d) {
...
test('signup rejects invalid usernames and reserved names', async () => {
  ...
  try {
    ...
    for (const u of ['abcd', 'has space', 'punct!', 'server', 'admin']) {
      const res = await signupUser(app, client, u, randomAesKey(), d);
      assert.equal(res.statusCode, 400, `username ${u} should be rejected`);
    }
  } finally {
    await teardown();
  }
...
test('signup rejects malformed keys', async () => {
  ...
  try {
    ...
    const badPub = await app.inject({
      method: 'POST',
      url: '/api/signup',
      payload: { u: 'alice', p: 'too-short', a: randomAesKey(), d, s: 'x' },
    });
...
test('challenge fails for unknown account or device', async () => {
  const { app, teardown } = await setupApp(LIMITS);
  try {
    const client = makeClient();
    const d = 'device-one-123';
    ...
    assert.equal(noUser.statusCode, 404);
...
test('canonical signup payload is what the client signs', async () => {
  ...
  const body = { a: 'AAA', d: 'device-one-123', p: 'BBB', u: 'alice' };
  assert.equal(
    canonical(body),
    canonical({ u: 'alice', p: 'BBB', d: 'device-one-123', a: 'AAA' }),
  );
});
